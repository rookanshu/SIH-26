...
export function WeatherView() {
  ...
  const [selectedWeatherId, setSelectedWeatherId] = useState<string>("wx-1");

  const activeWeather =
    MOCK_WEATHER.find((w) => w.id === selectedWeatherId) || MOCK_WEATHER[0];
    ...
    <div className="flex-1 flex flex-col min-h-0 bg-slate-950 p-3 sm:p-5 overflow-y-auto space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-2 border-b border-slate-800">
        ...
        <div className="flex items-center gap-2">
          <span className="text-[11px] font-semibold text-slate-400 px-2.5 py-1 rounded bg-slate-900 border border-slate-800">
            Precipitation Telemetry: <strong className="text-sky-400">Live 24h Gauge</strong>
          </span>
        </div>
      </div>
      ...
      <div className="space-y-2">
        <div className="text-xs font-bold uppercase tracking-wider text-slate-300">
          Regional Weather Stations & Logistics Telemetry ({MOCK_WEATHER.length})
        </div>
